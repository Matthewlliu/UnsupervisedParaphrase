...
class bart_model(object):
    ...
    def build_model(self, checkpoint_dir=None, with_tokenizer=False):
        if checkpoint_dir is None or with_tokenizer is False:
            self.tokenizer = BartTokenizer.from_pretrained('/data/MODELS/%s' % self.args.model)
            if self.special_tokens_dict is not None:
                self.tokenizer.add_special_tokens(self.special_tokens_dict)
            self.tokenizer.pad_token = self.tokenizer.eos_token
            logging.info("Load {} tokenizer".format(self.args.model))
        else:
            self.tokenizer = BartTokenizer.from_pretrained(checkpoint_dir)
            logging.info("Load tokenizer from {}".format(checkpoint_dir))

            
        if checkpoint_dir is None:
            self.model = BartForConditionalGeneration.from_pretrained('/data/MODELS/%s' % self.args.model)
            self.model.resize_token_embeddings(len(self.tokenizer))
            logging.info("Load {} model".format(self.args.model))
        else:
            self.model = BartForConditionalGeneration.from_pretrained(checkpoint_dir)
    ...
    def generate_text(self, input_texts, max_length=512, decoding='greedy', suffix=''):
        self.model.eval()
        sentences_list = []
        with torch.no_grad():
